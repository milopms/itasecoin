Itasecoin Core version 0.13.2 is now available from:

  <https://download.itasecoin.org/itasecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/itasecoin-project/itasecoin/issues>
...
updates will be released anymore. Without security updates, using a itasecoin
...
ECDSA signatures inside Itasecoin transactions now use validation using
...
Previous versions of Itasecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Itasecoin Core was to raise the default minimum
relay fee.

Itasecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Itasecoin Core 0.13.2 also introduces new default policy limits on the length and
...
Itasecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Itasecoin Core will relay transactions with
...
entirely in Itasecoin Core 0.13, and it is at this time undecided whether the
...
Itasecoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Itasecoin Core automatically creates a hidden service to listen on, without
manual configuration. Itasecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Itasecoin Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Itasecoin
Core to use floating fees. By default, Itasecoin Core will use floating
fees.
...
(default: `0.0002` ITASE/kB).

At all times, Itasecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ITASE.
Furthermore, Itasecoin Core will never create transactions paying less than
...
`itasecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
- Adding `dbcache=100` in itasecoin.conf
...
Various code modernizations have been done. The Itasecoin Core code base has
...
- `itasecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `itasecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Itasecoin scripting system that in combination with
...
Itasecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Itasecoin Core.
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Itasecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Itasecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Itasecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of itasecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of itasecoins being spent is exactly
...
- **Increased security for multisig:** Itasecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Itasecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Itasecoin's current ledger (called the UTXO set),
  ...
  note that Itasecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Itasecoin users to individually opt-in to almost any change in the Itasecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Itasecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Itasecoin
client to Itasecoin Core 0.13.2 to signal for segwit support. When a
...
Since Itasecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `itasecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `itasecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
